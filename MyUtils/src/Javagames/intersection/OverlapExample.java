...
public class OverlapExample extends SimpleFramework {
    ...
    protected void updateObject(float delta) {
        ...
        Matrix3x3f mat = Matrix3x3f.translate(rect0Pos);
        min0Cpy = mat.mul(min0);
        max0Cpy = mat.mul(max0);
        mat = Matrix3x3f.translate(rect1Pos);
        min1Cpy = mat.mul(min1);
        max1Cpy = mat.mul(max1);
        //position the circles
        mat = Matrix3x3f.translate(c0Pos);
        c0 = mat.mul(new Vector2f());
        mat = Matrix3x3f.translate(c1Pos);
        ...
        //一旦dragging为false，所有的Moving都被设置为false
    ...
    protected void render(Graphics g) {
        super.render(g);
        //render instructions
        g.drawString("Dragging: " + dragging, 30, 45);
        g.drawString("Click and hold to drag shapes", 30, 60);
        g.drawString("Press [SPACE] to reset", 30, 75);
        //render object
        drawAABB(g, min0Cpy, max0Cpy,rect0Collision ? Color.BLACK : Color.BLUE);
        drawAABB(g, min1Cpy, max1Cpy,rect1Collision ? Color.BLACK : Color.BLUE);
        drawCircle(g, c0, r0,circle0Collision ? Color.BLACK : Color.BLUE);
        drawCircle(g, c1, r1,circle1Collision ? Color.BLACK : Color.BLUE);
    }

    //draw the AABB
    private void drawAABB(Graphics g, Vector2f min, Vector2f max, Color color) {
        Vector2f topLeft = new Vector2f(min.x, max.y);
        Vector2f bottomRight = new Vector2f(max.x, min.y);
        Utility.drawRect(g, topLeft, bottomRight, getViewportTransform(), color);
    }

    //draw the circle
    private void drawCircle(Graphics g, Vector2f center, float radius, Color color) {
        Utility.drawOval(g, center, radius, getViewportTransform(), color);
    }
