...
public class LoadXMLExample {
    ...
    public void loadFile() {
        InputStream in = null;
        try {
            in = ResourceLoader.load(LoadXMLExample.class,
    ...
    /**
     * Element.getNodeName()：返回了XML标记的名称
     * Element.getTextContent()：返回了开始标记和结束标记之间的文本
     * Element.getAttribute(String name)：给定了属性名称，如果该属性可用的话，返回属性值或者null
     */
    private void parseXML(Document document) {
        Element element = document.getDocumentElement();
        List<Element> elements =
                XMLUtility.getAllElements(element, "empty");
        System.out.println("Elements: " + elements.get(0).getNodeName());
        ...
        System.out.println("Get-AllChildren-Count: " + allChildren.size());
