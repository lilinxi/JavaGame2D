...
public class PrototypeAsteroidFactory {
    //陨石工厂类
    //可以随机生成已经定义好的9中不同的陨石类的示例
    //可以选择生成指定大小的随机陨石或随机大小的随机陨石
    ...
    public PrototypeAsteroid createAsteroid(Vector2f position) {
        ...
        switch (randomSize) {
            case Small:
                return createSmallAsteroid(position);
            case Medium:
                return createMediumAsteroid(position);
            case Large:
                return createLargeAsteroid(position);
                default:
                    return null;
        }
    }
    ...
    private Vector2f[] mirror(Vector2f[] polygon) {
        ...
        mirror = Utility.transform(polygon, mat);
        return mirror;
    }
}